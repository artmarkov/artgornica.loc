...
use frontend\modules\parallax\ParallaxAsset;
 ...
 *
 */
class Parallax extends \yii\base\Widget
{
    ...
    // default from docs
    public $timer = ['class' => 'tp-bannertimer'];
    ...
    public function init()
    {
        ...
        $this->_module = \Yii::$app->getModule('parallax');
        $this->_pluginLocation = $this->_module->getPluginLocation();

        if ( $this->_module === null ) {
            throw new InvalidConfigException('Parallax Error: Module not found!');
        }

        if ( $this->_pluginLocation === null ) {
            throw new InvalidConfigException('Parallax Error: Plugin location not found!');
        }
        ...
        if ( ! isset($this->slides) ) {
            throw new InvalidConfigException('Parallax Error: You must have at least 1 slide!');
        }

        $asset = ParallaxAsset::register($this->view);
    ...
    public function buildJsScript()
    {
        ...
        if ( isset($this->wrapper['id']) ) {
            ...
            throw new InvalidConfigException('Parallax Error: Wrapper requires an id or a class!');
        }
    ...
    public function buildSlider()
    {
        ...
        $this->rawSliderHtml .= Html::beginTag('div', $this->timer);
        $this->rawSliderHtml .= Html::endTag('div');
    ...
    public function renderSlide($slide)
    {
        ...
        if ( ! isset($slide['image']) || ! isset($slide['image']['src']) || empty($slide['image']['src']) ) {
            throw new InvalidConfigException('Parallax Error: Missing slide image!');
        }
